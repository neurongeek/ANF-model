function [Spike, SpTimes, p_sptimes, c_sptimes] = Model_SinglePulse(Istim,p_Noise,c_Noise,Fs)
...
%
% Input required is Istim (stimulus waveform), Fs (sampling frequency) and
% Params (the parameter set). Params is a struct of parameter set for both
% peripheral and central neuron. Output of the function contains spike (a
% logical for a spike (Spike), spike time in Seconds (SpTime), the membrane
% voltage trace for peripheral neuron (P.v) and the membrane voltage trace
% for central neuron (C.v).
%
% Suyash N. Joshi
